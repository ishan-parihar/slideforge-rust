...
use palette::{FromColor, Oklch, Srgb, LinSrgb};
...
pub struct DesignTokens {
    ...
    pub body_font: String,
    pub google_fonts_url: String,
    pub type_scale: HashMap<String, TypeLevel>,
    pub spacing: HashMap<i32, i32>,
    pub contrast_report: HashMap<String, ContrastReportItem>,
...
pub fn generate_spacing_scale() -> HashMap<i32, i32> {
    let base = 8;
...
pub fn generate_type_scale(base_size: i32, ratio: f32) -> HashMap<String, TypeLevel> {
    let mut map = HashMap::new();
    map.insert("display".to_string(), TypeLevel {
        font_size: (base_size as f32 * ratio.powi(3)).round() as i32,
        line_height: 1.08,
        letter_spacing: -0.02,
        font_weight: 600,
    });
    map.insert("headline".to_string(), TypeLevel {
        font_size: (base_size as f32 * ratio.powi(2)).round() as i32,
        line_height: 1.12,
        letter_spacing: -0.015,
        font_weight: 600,
    });
    map.insert("title".to_string(), TypeLevel {
        font_size: (base_size as f32 * ratio.powi(1)).round() as i32,
        line_height: 1.18,
        letter_spacing: -0.01,
        font_weight: 600,
    });
    map.insert("body".to_string(), TypeLevel {
        font_size: base_size,
        line_height: 1.55,
        letter_spacing: 0.0,
        font_weight: 400,
    });
    map.insert("caption".to_string(), TypeLevel {
        font_size: (base_size as f32 / ratio).round() as i32,
        line_height: 1.40,
        letter_spacing: 0.02,
        font_weight: 600,
    });
    map.insert("micro".to_string(), TypeLevel {
        font_size: (base_size as f32 / ratio.powi(2)).round() as i32,
        line_height: 1.35,
        letter_spacing: 0.04,
        font_weight: 500,
...
pub fn get_font_pairing(style: &str) -> FontPairing {
    ...
    let (h_font, b_font, h_weights, b_weights, desc, h_class, b_class) = match style_clean.as_str() {
        "editorial" => ("Playfair Display", "DM Sans", vec![300, 600], vec![400, 500, 600], "Editorial / premium feel", "serif", "sans"),
        "warm" => ("Lora", "Nunito Sans", vec![400, 600], vec![400, 500, 600], "Warm / approachable", "serif", "sans"),
        "technical" => ("Space Grotesk", "Space Grotesk", vec![300, 600], vec![400, 500], "Technical / sharp", "sans", "sans"),
        "bold" => ("Fraunces", "Outfit", vec![300, 600], vec![400, 500, 600], "Bold / expressive", "serif", "sans"),
        "classic" => ("Libre Baskerville", "Work Sans", vec![400, 700], vec![400, 500, 600], "Classic / trustworthy", "serif", "sans"),
        "rounded" => ("Bricolage Grotesque", "Bricolage Grotesque", vec![600], vec![400, 500], "Rounded / friendly", "sans", "sans"),
        _ => ("Plus Jakarta Sans", "Plus Jakarta Sans", vec![700], vec![400, 500, 600], "Modern / clean", "sans", "sans"), // modern
    };
    
    let mut heading_families = vec![
        format!("{}:wght@{}", h_font, h_weights.iter().map(|w| w.to_string()).collect::<Vec<_>>().join(","))
    ];
    if h_font != b_font {
        heading_families.push(
            format!("{}:wght@{}", b_font, b_weights.iter().map(|w| w.to_string()).collect::<Vec<_>>().join(","))
        );
    }
    let families = heading_families.join("&family=");
    let url = format!("https://fonts.googleapis.com/css2?family={}&display=swap", families);
...
pub fn derive_palette(
    ...
    if contrast_ratio(&primary_light, &surface_dark) < 4.5 {
        primary_light = auto_clamp_text((pl + 0.15).min(0.97), pc, ph, &surface_dark, "lighten", 4.5);
    }
    
    let mut primary_dark = oklch_to_hex((pl - 0.15).max(0.05), pc, ph);
    if contrast_ratio(&primary_dark, &surface_light) < 4.5 {
        primary_dark = auto_clamp_text((pl - 0.15).max(0.05), pc, ph, &surface_light, "darken", 4.5);
    }
    
    if contrast_ratio(&primary_light, &primary_dark) < 4.5 {
        primary_light = auto_clamp_text((pl + 0.15).min(0.97), pc, ph, &primary_dark, "lighten", 4.5);
    }
    ...
    let temperature = if ph >= 180.0 { "warm".to_string() } else { "cool".to_string() };
    
    // Compile contrast report
    let mut contrast_report = HashMap::new();
    contrast_report.insert("text_primary on surface_light".to_string(), ContrastReportItem {
        ratio: (contrast_ratio(&text_primary, &surface_light) * 100.0).round() / 100.0,
        passes: passes_wcag_aa(&text_primary, &surface_light, false),
    });
    contrast_report.insert("text_secondary on surface_light".to_string(), ContrastReportItem {
    ...
    contrast_report.insert("text_on_dark on surface_dark".to_string(), ContrastReportItem {
        ratio: (contrast_ratio(&text_on_dark, &surface_dark) * 100.0).round() / 100.0,
        passes: passes_wcag_aa(&text_on_dark, &surface_dark, false),
    });
    contrast_report.insert("text_on_dark_secondary on surface_dark".to_string(), ContrastReportItem {
    ...
    contrast_report.insert("border_light on surface_light".to_string(), ContrastReportItem {
        ratio: (contrast_ratio(&border_light, &surface_light) * 100.0).round() / 100.0,
        passes: contrast_ratio(&border_light, &surface_light) >= 1.5,
    });
    contrast_report.insert("border_dark on surface_dark".to_string(), ContrastReportItem {
    ...
    shadows.insert("md".to_string(), "0 4px 6px -1px rgba(0,0,0,0.08), 0 2px 4px -2px rgba(0,0,0,0.05)".to_string());
    shadows.insert("lg".to_string(), "0 10px 15px -3px rgba(0,0,0,0.08), 0 4px 6px -4px rgba(0,0,0,0.05)".to_string());
    shadows.insert("xl".to_string(), "0 20px 25px -5px rgba(0,0,0,0.1), 0 8px 10px -6px rgba(0,0,0,0.05)".to_string());
    shadows.insert("inner".to_string(), "inset 0 2px 4px rgba(0,0,0,0.06)".to_string());
    ...
    gradients_map.insert("subtle".to_string(), format!("linear-gradient(135deg, {} 0%, {}08 100%)", surface_light, primary_str));
    ...
    gradients_map.insert("hero".to_string(), format!("linear-gradient(135deg, {} 0%, {}30 50%, {} 100%)", surface_dark, primary_dark, surface_dark));
    ...
    if let Some(over) = overrides {
        for (k, v) in over {
            let normalized = if v.starts_with("#") || v.len() == 6 {
                if let Ok(c) = parse_hex(v) { to_hex(&c) } else { v.to_string() }
            } else {
                v.to_string()
            };
...
fn revalidate_tokens(mut tokens: DesignTokens) -> Result<DesignTokens, String> {
    ...
    for (text_field, bg_field, is_large) in text_bg_pairs {
        ...
        if contrast_ratio(&fg_hex, &bg_hex) < threshold {
            let direction = if bg_hex == tokens.surface_light { "darken" } else { "lighten" };
...
pub fn get_contrast_safe_color(
    ...
    for bg in bg_colors {
        ...
        if let Some(caps) = hex_re.captures(bg_clean) {
            ...
            if h.len() == 8 {
                ...
                let fallback_bg = if bg_clean.contains("dark") || bg_clean.contains("gradient") || bg_clean.contains("hero") {
                    "#000000"
                } else {
                    "#FFFFFF"
                };
    ...
    // Determine direction
    let max_light_contrast = resolved_bg_list.iter()
        .map(|bg| contrast_ratio("#FFFFFF", bg))
        .fold(f32::INFINITY, f32::min);
    let max_dark_contrast = resolved_bg_list.iter()
        .map(|bg| contrast_ratio("#000000", bg))
        .fold(f32::INFINITY, f32::min);
        
    let direction = if max_light_contrast > max_dark_contrast { "lighten" } else { "darken" };
    ...
    for _ in 0..100 {
        let color_hex = oklch_to_hex(current_l, c, h);
        let worst_ratio = resolved_bg_list.iter()
