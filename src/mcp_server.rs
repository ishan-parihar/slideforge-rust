...
pub struct ConfigureDesignResponse {
    ...
    pub archetype: String,
    pub platform: String,
    pub contrast_report: HashMap<String, design_system::ContrastReportItem>,
    pub message: String,
}
...
impl Server {
    ...
    fn get_tokens(&self) -> Result<design_system::DesignTokens, ErrorData> {
        ...
        let style = if state.font_style.is_empty() { "modern".to_string() } else { state.font_style.clone() };
        let preset = if state.preset.is_empty() { "tonal_spot".to_string() } else { state.preset.clone() };
        ...
        design_system::derive_palette(&primary, &style, base, ratio, &preset, &visual_theme, None, None, None)
...
impl Server {
    ...
    /// Load the SlideForge capabilities guide.
    #[tool(name = "load_carousel_skill", description = "Load the SlideForge design guide: capabilities, slide types, themes, platforms, best practices.")]
    pub async fn load_carousel_skill(&self) -> Result<Json<String>, ErrorData> {
        Ok(Json(include_str!("../DESIGN-GUIDE.md").to_string()))
    }

    // ── configure_design ──────────────────────────────────────────────────────

    /// Configure design tokens and session brand settings. Call this first.
    #[tool(name = "configure_design", description = "Configure design tokens and session brand settings. Must be called before generating slides.")]
    pub async fn configure_design(
        ...
        let style = req.font_style.clone().unwrap_or_else(|| "modern".to_string());
        let preset = req.preset.clone().unwrap_or_else(|| "tonal_spot".to_string());
        let visual_theme = req.visual_theme.clone().unwrap_or_else(|| "editorial".to_string());
        ...
        state.layout_theme = req.layout_theme.clone().unwrap_or_else(|| "asymmetric".to_string());
        state.effect_theme = req.effect_theme.clone().unwrap_or_else(|| "glass".to_string());
        ...
        state.platform = req.platform.clone().unwrap_or_else(|| "instagram_portrait".to_string());
        ...
        Ok(Json(ConfigureDesignResponse {
            ...
            message: "Design system configured. All subsequent calls will use this configuration.".to_string(),
        }))
    }
    ...
    #[tool(name = "design_system", description = "Derive design tokens (palette, typography, spacing, effects) from a brand color. Stateless; does not update session.")]
    ...
    #[tool(name = "generate_slide", description = "Generate HTML for a single slide (hero, feature, list, quote, cta, comparison, stat_row, timeline, callout, split_features, grid_cards, headline_subheadline, definition, text_block).")]
    pub async fn generate_slide(
        ...
        let primary = req.primary_color.as_deref()
            .filter(|s| !s.is_empty())
            .map(|s| s.to_string())
        ...
        let style = req.font_style.clone()
            .filter(|s| !s.is_empty())
            .unwrap_or_else(|| if state.font_style.is_empty() { "modern".to_string() } else { state.font_style.clone() });
        let preset = req.preset.clone()
            .filter(|s| !s.is_empty())
            .unwrap_or_else(|| if state.preset.is_empty() { "tonal_spot".to_string() } else { state.preset.clone() });

        let bg_style = req.bg_style.clone()
            .or_else(|| Some(state.bg_style.clone()))
            .unwrap_or_else(|| "light".to_string());
        let bg_style = if bg_style.is_empty() { "light".to_string() } else { bg_style };

        let theme = req.theme.clone()
            .or_else(|| Some(state.visual_theme.clone()))
            .unwrap_or_else(|| "editorial".to_string());
        let theme = if theme.is_empty() { "editorial".to_string() } else { theme };

        let archetype = req.archetype.clone()
        ...
        let tokens = design_system::derive_palette(&primary, &style, base, ratio, &preset, &theme, None, None, None)
            .map_err(|e| ErrorData::internal_error(e, None))?;

        let params = req.params.unwrap_or(serde_json::json!({}));
        let slide_type = req.slide_type.to_lowercase().replace('-', "_");

        let result = components::dispatch_slide(&slide_type, &tokens, &params, &bg_style, &theme, &archetype)
    ...
    #[tool(name = "render_carousel", description = "Assemble slide HTML objects into a full carousel HTML document with CSS, fonts, and brand footer.")]
    pub async fn render_carousel(
        ...
        let css_vars = req.css_variables.clone()
            .filter(|s| !s.is_empty())
        ...
        if let Some(ref path) = output_path {
            std::fs::write(path, &html)
                .map_err(|e| ErrorData::internal_error(format!("Failed to write HTML: {}", e), None))?;
        }

        Ok(Json(RenderCarouselResponse { html, output_path, total_slides: total }))
    }

    // ── export_carousel_slides ────────────────────────────────────────────────

    /// Export carousel HTML to per-slide PNG images via headless Chrome.
    #[tool(name = "export_carousel_slides", description = "Render carousel HTML to PNG images using headless Chrome. Returns file paths to each exported slide.")]
    pub async fn export_carousel_slides(
        ...
        let preset_name = req.preset.clone().unwrap_or_else(|| {
            ...
            if p.is_empty() { "instagram_portrait".to_string() } else { p }
        });
        ...
        let paths = export::export_slides(&req.html_path, &req.output_dir, req.total_slides, width, height)
            .await
            .map_err(|e| ErrorData::internal_error(e, None))?;

        let total = paths.len();
        let dimensions = format!("{}×{}", width, height);
        Ok(Json(ExportResponse { exported_slides: paths, dimensions, preset: preset_name, total_exported: total }))
    }

    // ── list_slide_types ──────────────────────────────────────────────────────

    /// List all available slide types with metadata.
    #[tool(name = "list_slide_types", description = "List all available slide types with descriptions, required params, optional params, and variants.")]
    pub async fn list_slide_types(&self) -> Result<Json<serde_json::Value>, ErrorData> {
        Ok(Json(slide_registry::get_registry()))
    }

    // ── get_slide_type_info ───────────────────────────────────────────────────

    /// Get detailed info for a specific slide type.
    #[tool(name = "get_slide_type_info", description = "Get detailed info for one slide type: required params, optional params, variants, examples.")]
    ...
    #[tool(name = "get_slide_types_for_context", description = "Get recommended slide types for a context: opening, closing, data, social-proof, features, process.")]
    ...
    #[tool(name = "list_platforms", description = "List all export platforms (Instagram, TikTok, LinkedIn, etc.) with pixel dimensions and aspect ratios.")]
    pub async fn list_platforms(&self) -> Result<Json<serde_json::Value>, ErrorData> {
        let all = platforms::all_platforms();
        let list: Vec<serde_json::Value> = all.iter().map(|p| serde_json::json!({
            "name": p.name,
            "width": p.width,
            "height": p.height,
        ...
        Ok(Json(serde_json::json!({ "platforms": list })))
    }

    // ── list_archetypes ───────────────────────────────────────────────────────

    /// List all available brand archetypes.
    #[tool(name = "list_archetypes", description = "List all brand archetypes (educator, thought_leader, startup_pitch, creator, etc.) with their default styling.")]
    pub async fn list_archetypes(&self) -> Result<Json<serde_json::Value>, ErrorData> {
        let all = archetypes::all_archetypes();
        let list: Vec<serde_json::Value> = all.iter().map(|a| serde_json::json!({
            "name": a.name,
            "description": a.description,
            "primary_theme": a.primary_theme,
            "default_bg_style": a.default_bg_style,
        })).collect();
        Ok(Json(serde_json::json!({ "archetypes": list })))
    }

    // ── validate_layout ───────────────────────────────────────────────────────

    /// Validate slide params before generating HTML.
    #[tool(name = "validate_layout", description = "Validate slide parameters before rendering. Returns errors for missing required fields and warnings for common issues.")]
    ...
    #[tool(name = "validate_and_fix", description = "Validate slide params and auto-fix missing optional fields with sensible defaults. Returns fixed params.")]
    ...
    #[tool(name = "list_themes", description = "List all available visual themes (editorial, bold, minimal, dark, vibrant, natural) with style descriptions.")]
    pub async fn list_themes(&self) -> Result<Json<serde_json::Value>, ErrorData> {
        Ok(Json(serde_json::json!({
            "themes": [
                {
                    "name": "editorial",
    ...
    #[tool(name = "recommend_color_scheme", description = "Generate harmonious color scheme options from a primary brand color using OKLCH perceptual color science.")]
    pub async fn recommend_color_scheme(
        ...
        let presets = ["tonal_spot", "vibrant", "neutral", "monochrome", "expressive", "fidelity"];

        let mut schemes = Vec::new();
        for &p in presets.iter().take(num) {
            if let Ok(tokens) = design_system::derive_palette(
                &req.primary_color, style, 16, 1.25, p, "", None, None, None,
            ) {
