...
enum Commands {
    ...
    SkillGuide {
        /// Verify the committed SKILL.md matches the live CLI command surface (CI gate, exit 1 when stale)
        #[arg(long)]
        check: bool,
        /// Regenerate SKILL.md from the live CLI command surface
        #[arg(long)]
        write: bool,
    },
...
body {{ margin:0; padding:0; background:#f0f0f0; display:flex; justify-content:center; align-items:center; min-height:100vh; }}
        ...
        Some(Commands::SkillGuide { check, write }) => {
            if *write {
...
mod tests {
    ...
    #[test]
    fn test_render_skill_md_covers_cli_surface() {
        let md = render_skill_md();
        // Every CLI subcommand is documented (drift-proof command table).
        ...
        // Removed slide types never appear in the catalog. Parse the
        // comma-separated Slide Types line into a set and assert membership
        // (position-agnostic — catches a removed type anywhere in the list).
        let lines: Vec<&str> = md.lines().collect();
        let types_line = lines
            .iter()
            .position(|l| l.trim() == "## Slide Types")
            .and_then(|i| lines.get(i + 1))
            .copied()
            .unwrap_or("");
        let listed: Vec<&str> = types_line.split(',').map(|s| s.trim()).collect();
        for removed in [
            ...
            assert!(
                ...
                "SKILL.md must not list removed type '{}'",
                removed
            );
        }
        // Active types do appear.
        for active in ["hero", "metric_grid", "process_map"] {
            assert!(md.contains(active), "SKILL.md must list active type '{}'", active);
        }
        // Platform catalog is live.
        assert!(md.contains("instagram_portrait"), "platforms section present");
    }
}

// ── AXI §7: SKILL.md single source of truth ────────────────────────────────

/// Render the authoritative SKILL.md from the live CLI surface (clap
/// subcommands + slide-type/platform catalogs), so the skill can never drift
/// from the binary. `skill-guide --check` fails CI when the committed SKILL.md
/// is stale; `skill-guide --write` regenerates it.
...
"#,
    ...
    let cli_cmd = Cli::command();
    let mut subs: Vec<(String, String)> = cli_cmd
        .get_subcommands()
        .map(|s| {
            (
    ...
    let active_types: Vec<String> = slide_registry::list_slide_types()
        .into_iter()
        .filter(|t| match slide_registry::get_slide_type_info(t) {
            // Hard-removed types (feature, list, callout, ...) surface None.
            None => false,
            // Deprecated aliases still route but are marked layout_family removed.
            Some(i) => i
                .get("layout_family")
                .and_then(|v| v.as_str())
                .map(|f| f != "removed")
    ...
    let plats: Vec<String> = platforms::all_platforms()
        .iter()
...
/// Verify the committed SKILL.md matches the live CLI surface (AXI §7 single
/// source of truth). Exits 1 when stale so CI fails; prints a TOON report.
fn skill_check() -> Result<(), Box<dyn std::error::Error>> {
    let rendered = render_skill_md();
    match fs::read_to_string("SKILL.md") {
        Ok(committed) => {
            if committed == rendered {
                println!("skill: ok (SKILL.md matches the live CLI surface)");
            } else {
                let committed_lines: Vec<&str> = committed.lines().collect();
                let rendered_lines: Vec<&str> = rendered.lines().collect();
                let mut detail = format!(
                    "line count differs: committed {} vs generated {}",
                    committed_lines.len(),
                    rendered_lines.len()
                );
                for (i, (a, b)) in committed_lines.iter().zip(rendered_lines.iter()).enumerate() {
                    if a != b {
                        detail = format!("line {}: committed `{}` vs generated `{}`", i + 1, a, b);
                        break;
                    }
                }
                output_toon(&json!({
